...
@genType
and entities = {
    @as("Activity") activity: entityStoreOperations<Entities.Activity.t>,
    @as("BondToken") bondToken: entityStoreOperations<Entities.BondToken.t>,
  ...
  let entities = {
      activity: {
        makeStoreOperatorEntity(
          ...
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.Activity)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.Activity.t,
            >
      ...
      bondToken: {
        makeStoreOperatorEntity(
          ...
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.BondToken)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.BondToken.t,
            >
          ),
          ~getKey=({id}) => id,
        )
